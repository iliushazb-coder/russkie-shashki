...
let myTelegramPhotoUrl = "";
let myTelegramUsername = "";

// Trusted remote Telegram usernames live outside room metadata.
// Base/production rules had no telegramProfiles write grant, so legacy clients
// could not pre-seed this namespace before the hardened rules exist.
const verifiedTelegramUsernameCache = Object.create(null);
const verifiedTelegramUsernameLoads = Object.create(null);
...
const translations = {
    ru: {
        ...
        status_confirming_result: "Подтверждение…",
    ...
    en: {
        ...
        btn_back: "Back",
        waiting_link_prompt: "Invitation ready",
        btn_share_link: "📤 Send to friend",
        btn_copy_link: "📋 Copy link",
        invite_link_copied: "✓ Link copied",
        invite_link_copy_failed: "Could not copy link",
        ...
        status_confirming_result: "Confirming…",
    ...
    it: {
        ...
        status_confirming_result: "Conferma…",
...
const waitingInvitePrompt = document.getElementById("waiting-invite-prompt");
...
function getAppScreens() {
    return [
        menuScreen,
        timeControlScreen,
        document.getElementById("group-lobby-screen"),
        waitingScreen,
        gameScreen
    ].filter(Boolean);
}
...
function showScreen(screen) {
    ...
    // №8 presentation-only reset: каждый новый вход на waiting-screen
    // начинается в нейтральном состоянии. Invite/create/cancel логика не
    // знает о декоративном success-классе и не зависит от него.
    if (screen === waitingScreen) waitingScreen.classList.remove("waiting-connected");
...
// ===== ИГРОВОЙ ДВИЖОК (№22: вынесен в shared/game-engine.js) =====
// Единый implementation source для client и Worker. Здесь — только
// bindings на реально используемые в script.js primitives; сама логика
// (createInitialPieces...computeNextDrawState) больше не дублируется тут.
if (!window.RussianCheckersEngine || typeof window.RussianCheckersEngine.attemptMove !== "function") {
    throw new Error("RussianCheckersEngine failed to load");
}
const {
    createInitialPieces,
    pieceAt,
...
// ===== №7 BEAUTY: TIMER + TURN — чисто presentation-state =====
// Последний реально отрисованный ход нужен только для одноразового cue.
// Он НЕ пишется в Firebase и НЕ участвует в игровой логике.
let lastTurnPresentation = null;
...
const CAPTURE_EFFECT_DURATION_MS = 260;
...
function gamePanelPlayerForColor(color) {
    const stored = (currentState && currentState.players && currentState.players[color]) || null;

    // Owner bot-game keeps compact player data. Enrich only the render object
    // with the already verified Telegram photo; never write it into gameplay state.
    if (isBotGame && !isSpectator) {
        if (color === botColor) {
            return Object.assign({}, stored || {}, {
                id: "bot",
                name: (stored && stored.name) || "🤖 Компьютер"
            });
        }
        if (color === myColor) {
            return makeTelegramPlayer(
                myTelegramId || (stored && stored.id) || "",
                (stored && stored.name) || myTelegramName || "Игрок",
                myTelegramPhotoUrl
            );
        }
    }

    if (stored && stored.id === myTelegramId) {
        const enrichment = {};
        if (!stored.photoUrl && myTelegramPhotoUrl) enrichment.photoUrl = myTelegramPhotoUrl;
        if (Object.keys(enrichment).length) return Object.assign({}, stored, enrichment);
    }
    return stored;
}

function compactGamePlayerName(name) {
    const raw = String(name || "").trim();
    const withoutAt = raw.charAt(0) === "@" ? raw.slice(1) : raw;
    const chars = Array.from(withoutAt);
    return chars.length > 4 ? chars.slice(0, 4).join("") + "." : withoutAt;
}

function gamePanelTelegramUsername(player, rawName, trustedUsername) {
    if (player && player.id === "bot") return BOT_USERNAME;
    // Never trust room player.username here: old production rules allowed
    // unknown player fields, so legacy data may contain an unverified value.
    const username = normalizedTelegramUsername(trustedUsername);
    const name = String(rawName || "").trim();
    return username && name === "@" + username ? username : "";
}

function openGamePanelTelegramProfile(event, username) {
    if (!username) return;
...
function renderPlayerNameCell(cell, marker, player, name, rating, ratingCell, trustedUsername) {
    if (cell) {
        const username = gamePanelTelegramUsername(player, name, trustedUsername);
        const tagName = username ? "a" : "span";
        let identity = cell.querySelector(".game-player-identity");

        if (!identity || identity.tagName.toLowerCase() !== tagName) {
            cell.textContent = "";
            identity = document.createElement(tagName);
            identity.className = "game-player-identity";
            cell.appendChild(identity);
        }

        identity.textContent = "";
        identity.className = "game-player-identity" + (username ? " game-player-identity-link" : "");
        identity.title = String(name || "");

        if (username) {
            identity.href = "https://t.me/" + username;
            identity.target = "_blank";
            identity.rel = "noopener noreferrer";
            identity.setAttribute("aria-label", String(name || ""));
            identity.onclick = function (event) {
                openGamePanelTelegramProfile(event, username);
            };
        } else {
            identity.removeAttribute("aria-label");
            identity.onclick = null;
        }
        ...
        fallback.className = "game-player-avatar-fallback";
        fallback.textContent = lobbyAvatarFallback(name, player && player.id);
        avatar.appendChild(fallback);

        const photoUrl = safeLobbyTelegramPhotoUrl(player && player.photoUrl);
        if (photoUrl) {
            const img = document.createElement("img");
            img.className = "game-player-avatar-img";
            img.src = photoUrl;
            img.alt = "";
            img.loading = "lazy";
            img.decoding = "async";
        ...
        label.textContent = compactGamePlayerName(name);
...
function getTurnPresentationGenerationKey() {
    if (!currentState) return null;
    if (isBotGame) return "bot:" + (currentBotMatchId || "local");
    return "online:" + ratedGenerationKey(roomCode || "local", currentState.matchNumber, currentState.createdAt);
}

function renderTurnPanelPresentation(topColor, bottomColor) {
    const turn = (!currentState || currentState.winner) ? null : currentState.turn;

    const hasTurn = turn === "light" || turn === "dark";

    // Активная и неактивная панели намеренно контрастируют: чей ход —
    // яркий, вторая сторона — приглушённая. Это presentation-only и одинаково
    // работает для игрока и зрителя, без семантики "твой ход".
    playerTopPanel.classList.toggle("turn-active", hasTurn && turn === topColor);
    playerBottomPanel.classList.toggle("turn-active", hasTurn && turn === bottomColor);
    playerTopPanel.classList.toggle("turn-inactive", hasTurn && turn !== topColor);
    playerBottomPanel.classList.toggle("turn-inactive", hasTurn && turn !== bottomColor);

    if (!hasTurn) {
        playerTopPanel.classList.remove("turn-cue");
        playerBottomPanel.classList.remove("turn-cue");
        lastTurnPresentation = null;
        return;
    }

    const generation = getTurnPresentationGenerationKey();
    const moveCount = (typeof currentState.moveCount === "number") ? currentState.moveCount : 0;
    const previous = lastTurnPresentation;

    // Сначала запоминаем новый snapshot. Первый render нового поколения
    // (первый вход, reload/reconnect, реванш) НЕ изображает событие смены хода.
    lastTurnPresentation = { generation: generation, turn: turn, moveCount: moveCount };
    ...
    // Presence/rating re-render того же состояния ничего не перезапускает.
    // При multi-capture moveCount растёт на каждом прыжке, но turn остаётся
    // тем же — поэтому cue появляется только при настоящей смене стороны.
    if (previous.turn === turn) return;

    // Optimistic online-ход может успеть визуально переключить turn, а затем
    // быть отклонён и заменён серверным snapshot с меньшим moveCount и прежним
    // turn. Это НЕ новая смена хода: снимаем уже запущенный cue с неверной
    // панели и не запускаем новый на стороне, к которой произошёл rollback.
    if (moveCount <= previous.moveCount) {
        playerTopPanel.classList.remove("turn-cue");
        playerBottomPanel.classList.remove("turn-cue");
        return;
    }

    const targetPanel = (turn === topColor)
        ? playerTopPanel
        : ((turn === bottomColor) ? playerBottomPanel : null);
    if (!targetPanel) return;

    playerTopPanel.classList.remove("turn-cue");
    playerBottomPanel.classList.remove("turn-cue");
    // Если тот же panel получает cue снова через один полный ход, класс мог
    // остаться после прошлой одноразовой CSS-анимации. Reflow нужен только
    // для корректного повторного запуска presentation-эффекта.
    void targetPanel.offsetWidth;
    targetPanel.classList.add("turn-cue");
}
...
function renderPlayerPanels() {
    ...
    // это внешние данные.
    const topPlayer = gamePanelPlayerForColor(topColor);
    const bottomPlayer = gamePanelPlayerForColor(bottomColor);
    const topTrustedUsername = trustedGamePanelUsernameForPlayer(topPlayer);
    const bottomTrustedUsername = trustedGamePanelUsernameForPlayer(bottomPlayer);

    renderPlayerNameCell(playerTopName,
        topColor,
        topPlayer,
        (topColor === "light" ? lightName : darkName),
        topRating, playerTopRating, topTrustedUsername);
    renderPlayerNameCell(playerBottomName,
        bottomColor,
        bottomPlayer,
        (bottomColor === "light" ? lightName : darkName),
        bottomRating, playerBottomRating, bottomTrustedUsername);
    ...
    renderTurnPanelPresentation(topColor, bottomColor);
...
// Длительность подбиралась владельцем вручную: 340 -> 680 -> 1500 -> 1000.
// Последнее значение не «ускорение эффекта», а сохранение ТЕМПА при
// переходе с 540 градусов на 360: 1500 * 360 / 540 = 1000, то есть один
// переворот занимает столько же времени, сколько занимал раньше.
//
...
// считает от него же.
const KING_PROMOTION_DURATION_MS = 1000;
...
function kingPromotionSoundDelayMs() {
    // В игре с ботом визуального превращения больше нет, поэтому ждать
    // нечего: задержка существует исключительно ради попадания последней
    // ноты в момент появления дамки. Без анимации она обернулась бы почти
    // секундой тишины после уже готовой дамки -- ровно та же причина, по
    // которой задержка снимается при prefers-reduced-motion.
...
function playKingPromotionEffect() {
    ...
    // В играх с ботом визуального превращения нет вовсе -- решение
    // владельца после ручного теста. Критерий именно isBotGame, а не цвет
    // или владелец хода: и превращение бота, и превращение человека
    // против бота, и обычное, и внутри серии взятий -- все без анимации.
    // Дамка просто появляется по authoritative-состоянию.
    //
    // ЗВУК это не затрагивает: он живёт отдельно, через
    // playSoundForMoveType в четырёх точках вызова, и ни одна из них не
...
function renderBoard() {
    ...
    if (isBotGame && currentState && !currentState.winner && currentState.turn === botColor) {
        ...
        //
        // Отдельной паузы для превращения больше нет. Она добавлялась,
        // чтобы анимация превращения бота успела досмотреться до конца --
        // но визуального превращения в играх с ботом теперь не бывает
        // вовсе, и ждать стало нечего. Мёртвую ветку не оставляем.
        if (!botMoveTimer) {
...
function updateTimerDisplay() {
    // #turn-timer также показывает sync-status. Снимаем low-time класс ДО
    // любого early return, чтобы тёплое breathing-состояние не "прилипало"
    // к «Отправляю ход…», закончившейся партии или игре без таймера.
    turnTimerDiv.classList.remove("timer-low");
    ...
    if (remaining <= 10) turnTimerDiv.classList.add("timer-low");
}
...
// ===== №5: ФИНАЛЬНОЕ ВИЗУАЛЬНОЕ НАСТРОЕНИЕ ИТОГА =====
// Чисто визуальная классификация: online по UID, bot по myColor,
// spectator/postоронний — neutral. Никакой игровой логики не меняет.
const END_GAME_MOOD_CLASSES = [
    "end-game-mood-win",
    "end-game-mood-loss",
    "end-game-mood-draw",
    "end-game-mood-neutral"
];

function getEndGameVisualMood() {
    if (!currentState || !currentState.winner) return null;
    if (currentState.winner === "draw") return "draw";
    ...
    if (winnerUid || loserUid) {
        if (myTelegramId === winnerUid) return "win";
        if (myTelegramId === loserUid) return "loss";
        return "neutral";
    }
...
function applyEndGameVisualMood() {
    if (!endGameModal) return;

    END_GAME_MOOD_CLASSES.forEach(function (className) {
        endGameModal.classList.remove(className);
    });

    const mood = getEndGameVisualMood();
    if (mood) endGameModal.classList.add("end-game-mood-" + mood);
}

function renderEndGameModal() {
    applyEndGameVisualMood();
    if (currentState && currentState.winner) {
        if (currentState.winner === "draw") {
...
function mirrorCommittedStateToSpectateRoom(spectateCode, committedSession) {
    ...
    const humanPlayer = makeTelegramPlayer(myTelegramId, myTelegramName || "Игрок", myTelegramPhotoUrl, myTelegramUsername);
...
function startBotSpectateRoom() {
    ...
    const botPlayer = { id: "bot", name: "🤖 Компьютер" };
    const humanPlayer = makeTelegramPlayer(myTelegramId, myTelegramName, myTelegramPhotoUrl, myTelegramUsername);
...
function createOnlineRoom() {
    ...
    database.ref("users/" + myTelegramId + "/activeMatch").remove().then(function () {
        ...
        const initialState = {
            ...
            moveType: null,
            pieces: createInitialPieces(),
            players: { light: makeTelegramPlayer(myTelegramId, myTelegramName, myTelegramPhotoUrl, myTelegramUsername), dark: null },
            timeControlSeconds: 0,
            turnStartedAt: firebase.database.ServerValue.TIMESTAMP,
            winner: null,
            winReason: null,
...
function attachFriendWaitingStatusListener(code) {
    ...
    friendWaitingStatusHandler = function (snapshot) {
        ...
        waitingScreen.classList.add("waiting-connected");
...
let copyInviteFeedbackTimer = null;

function getPendingFriendInviteLink() {
    if (!myPendingFriendRoomCode) return "";
    return "https://t.me/" + BOT_USERNAME + "?startapp=" + myPendingFriendRoomCode;
}

function resetCopyInviteButton() {
    if (!btnCopyLink) return;
    if (copyInviteFeedbackTimer) {
        clearTimeout(copyInviteFeedbackTimer);
        copyInviteFeedbackTimer = null;
    }
    btnCopyLink.textContent = t("btn_copy_link");
}

function showCopyInviteFeedback(key) {
    if (!btnCopyLink) return;
    const codeAtClick = myPendingFriendRoomCode;
    if (copyInviteFeedbackTimer) clearTimeout(copyInviteFeedbackTimer);
    btnCopyLink.textContent = t(key);
    copyInviteFeedbackTimer = setTimeout(function () {
        copyInviteFeedbackTimer = null;
        if (myPendingFriendRoomCode !== codeAtClick) return;
        btnCopyLink.textContent = t("btn_copy_link");
    }, 1400);
}
...
function copyPendingFriendInviteLink() {
    const link = getPendingFriendInviteLink();
    if (!link) return;
    ...
    if (clipboard) {
        clipboard.writeText(link).then(function () {
            showCopyInviteFeedback("invite_link_copied");
        }).catch(function () {
            showCopyInviteFeedback(fallbackCopyInviteLink(link)
                ? "invite_link_copied"
...
if (btnCopyLink) {
    btnCopyLink.addEventListener("click", copyPendingFriendInviteLink);
}
...
setInterval(function () {
    if (isScreenLogicallyActive(gameScreen)) {
        updateTimerDisplay();
        checkTimeout();
        updatePresenceOnly();
...
function checkForInviteLink() {
    ...
    roomCode = startParam;
    waitingInvitePrompt.classList.add("hidden");
    ...
    database.ref("rooms/" + roomCode).once("value").then(function (snapshot) {
        ...
        function startAtomicInviteJoin() {
            ...
            return new Promise(function (resolve) {
                ...
                // invite-join.test.js).
                if (!canUseFirebase()) return Promise.resolve(null);
                return attemptAtomicInviteJoin(inviteRoomRef, latestWarmRoom, myTelegramId, myTelegramName, myTelegramPhotoUrl, myTelegramUsername);
            }).then(function (result) {
                detachWarmListener();
...
function renderOnlineStatsRow(rank, name, wins, losses, draws, rating, isSelf) {
    const row = document.createElement("div");
    row.className = "stats-row";
    if (rank >= 1 && rank <= 3) {
        row.classList.add("stats-row-podium");
        row.classList.add("stats-row-rank-" + rank);
    }
    if (isSelf) row.classList.add("stats-row-self");
...
function renderBotStatsRow(rank, name, wins, losses, byLevel, isSelf) {
    const row = document.createElement("div");
...
function openStatsModal() {
    ...
    const onlineStatsLoaded = Promise.all([
        ...
        top.forEach(function (entry, index) {
            statsLeaderboard.appendChild(renderOnlineStatsRow(
                index + 1,
                entry.name,
                entry.wins,
                entry.losses,
    ...
    if (statsLeaderboardBot) {
        ...
        botStatsLoaded = database.ref("statsBot").once("value").then(function (snapshot) {
            ...
            top.forEach(function (entry, index) {
                statsLeaderboardBot.appendChild(renderBotStatsRow(
                    index + 1,
                    entry.name,
                    entry.wins,
                    entry.losses,
...
function startFirebaseFlows(me) {
    ...
    myTelegramId = me.id; myTelegramName = me.name; myTelegramPhotoUrl = (typeof me.photoUrl === "string") ? me.photoUrl : ""; myTelegramUsername = normalizedTelegramUsername(me.username); firebaseAuthReady = true; firebaseFlowsStarted = true; pendingFirebaseIdentity = null;

    // Publish only the claim-bound username into a namespace that legacy
    // production rules could not write. Removing is allowed when Telegram
    // no longer supplies a username, so stale mappings do not linger.
    const verifiedProfileRef = database.ref("telegramProfiles/" + myTelegramId);
    const verifiedProfileWrite = myTelegramUsername
        ? verifiedProfileRef.set(myTelegramUsername)
        : verifiedProfileRef.remove();
    if (verifiedProfileWrite && typeof verifiedProfileWrite.catch === "function") {
        verifiedProfileWrite.catch(function (error) {
            console.warn("Verified Telegram profile sync failed:", error);
        });
    }
...
function makeTelegramPlayer(id, name, photoUrl, username) {
    const player = { id: id, name: name };
    if (typeof photoUrl === "string" && photoUrl) player.photoUrl = photoUrl;
    const safeUsername = normalizedTelegramUsername(username);
    if (safeUsername) player.username = safeUsername;
    return player;
}

function trustedGamePanelUsernameForPlayer(player) {
    if (!player) return "";
    if (player.id === "bot") return BOT_USERNAME;
    const uid = typeof player.id === "string" ? player.id : "";
    if (!/^tg_\d+$/.test(uid)) return "";
    if (uid === myTelegramId) return myTelegramUsername;

    const hasCached = Object.prototype.hasOwnProperty.call(verifiedTelegramUsernameCache, uid);
    if (!hasCached) ensureVerifiedTelegramUsername(uid);
    return hasCached ? verifiedTelegramUsernameCache[uid] : "";
}

function ensureVerifiedTelegramUsername(uid) {
    if (!canUseFirebase() || typeof uid !== "string" || !/^tg_\d+$/.test(uid) || uid === myTelegramId) return;
    if (Object.prototype.hasOwnProperty.call(verifiedTelegramUsernameCache, uid) || verifiedTelegramUsernameLoads[uid]) return;

    verifiedTelegramUsernameLoads[uid] = true;
    let loaded = false;
    database.ref("telegramProfiles/" + uid).once("value").then(function (snapshot) {
        verifiedTelegramUsernameCache[uid] = normalizedTelegramUsername(snapshot.val());
        loaded = true;
    }).catch(function (error) {
        console.warn("Verified Telegram profile read failed:", error);
    }).then(function () {
        delete verifiedTelegramUsernameLoads[uid];
        if (!loaded || !currentState || !currentState.players) return;
        const light = currentState.players.light;
        const dark = currentState.players.dark;
        if ((light && light.id === uid) || (dark && dark.id === uid)) renderPlayerPanels();
    });
}

function safeLobbyTelegramPhotoUrl(value) {
    ...
    try {
        ...
        return trustedHost ? url.href : "";
    } catch (e) {
        return "";
    }
}

function lobbyAvatarFallback(name, playerId) {
    if (playerId === "bot") return "🤖";
    const cleaned = String(name || "").trim().replace(/^@/, "");
    return cleaned ? Array.from(cleaned)[0].toUpperCase() : "●";
}

function buildLobbyAvatarMarkup(player, rawName) {
    const photoUrl = safeLobbyTelegramPhotoUrl(player && player.photoUrl);
    const fallback = escapeHtml(lobbyAvatarFallback(rawName, player && player.id));
    const image = photoUrl
        ? '<img class="group-room-avatar-img" src="' + escapeHtml(photoUrl) + '" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer">'
        : "";
    return '<span class="group-room-avatar" aria-hidden="true"><span class="group-room-avatar-fallback">' + fallback + '</span>' + image + '</span>';
}

function claimDarkSeatAndActivate(roomRef, uid, name, photoUrl, username) {
    return roomRef.update({
        "players/dark": makeTelegramPlayer(uid, name, photoUrl, username),
        status: "active",
        turnStartedAt: firebase.database.ServerValue.TIMESTAMP
    });
}
...
function attemptAtomicInviteJoin(roomRef, warmRoom, uid, name, photoUrl, username) {
    ...
    return claimDarkSeatAndActivate(roomRef, uid, name, photoUrl, username).then(function () {
...
function renderLobbyListFromCache() {
    ...
    for (const code in lobbyRoomsByCode) {
        ...
        // пустого слота.
        const lightPlayer = room.players && room.players.light;
        const darkPlayer = room.players && room.players.dark;
        const lightNameRaw = (lightPlayer && lightPlayer.name) || t("waiting_for_opponent");
        const darkNameRaw = (darkPlayer && darkPlayer.name) || t("waiting_for_opponent");
        const lightAvatar = buildLobbyAvatarMarkup(lightPlayer, lightNameRaw);
        const darkAvatar = buildLobbyAvatarMarkup(darkPlayer, darkNameRaw);
        let lightName = escapeHtml(lightNameRaw);
        let darkName = escapeHtml(darkNameRaw);
            ...
            if (isMyActiveGame) {
                activeHtml += `
                    <div class="group-room-card">
                        <div class="group-room-info active">
                            <span class="group-room-player group-room-player-light">${lightAvatar}<span class="group-room-piece" aria-hidden="true">⚫</span><span class="group-room-name">${lightName}</span></span>
                            <span class="group-room-vs">vs</span>
                            <span class="group-room-player group-room-player-dark"><span class="group-room-name">${darkName}</span><span class="group-room-piece" aria-hidden="true">⚪</span>${darkAvatar}</span>
                        </div>
    ...
    if (!finalHtml) {
        finalHtml = '<p class="section-title lobby-empty-state">' + t("lobby_empty") + '</p>';
    }
    groupRoomsList.innerHTML = finalHtml;

    groupRoomsList.querySelectorAll('.group-room-avatar-img').forEach(function (img) {
        img.addEventListener('error', function () {
...
function joinGroupRoom(code) {
    ...
    database.ref("rooms/" + roomCode).once("value").then(function(snapshot) {
        ...
        // turnStartedAt.
        claimDarkSeatAndActivate(database.ref("rooms/" + roomCode), myTelegramId, myTelegramName, myTelegramPhotoUrl, myTelegramUsername).then(function() {
            if (!canUseFirebase()) return;
...
async function authenticateTelegramUser() {
    ...
    return {
        ...
        username: normalizedTelegramUsername(payload.username)
    };
}
...
//
// Normal launch: cover должен жить не "по таймеру", а до реальной
// готовности приложения. При этом сохраняем короткий минимум, чтобы две
// шашки успевали читаться даже на очень быстром устройстве.
//
// Важно: minimum timer НЕ await'ится и не тормозит authenticateTelegramUser().
// Cover снимается только когда выполнены ОБА условия:
//   1) прошёл minimum visual window;
//   2) auth завершился и Firebase-flow уже запущен.
// Если auth медленный, cover автоматически остаётся дольше.
const NORMAL_STARTUP_COVER_MIN_MS = 800;
...
async function bootstrapApp() {
    ...
    authPromise = (async function () {
        ...
        // После тех же 100 мс start_param уже можно надёжно различить.
        // Invite-flow живёт по своему существующему lifecycle и НЕ получает
        // normal minimum timer. Для обычного запуска запускаем независимый
        // visual timer и ждём auth параллельно.
        const inviteLaunch = hasInviteIntent();
        let normalMinElapsed = false;
        let normalAppReady = false;

        function maybeHideNormalStartupCover() {
            if (!inviteLaunch && normalMinElapsed && normalAppReady) {
                hideStartupCover();
            }
        }
        ...
        if (inviteLaunch) {
            ...
            setTimeout(function () {
                normalMinElapsed = true;
                maybeHideNormalStartupCover();
            }, NORMAL_STARTUP_COVER_MIN_MS);
        }
        ...
        // Здесь normal app уже прошёл auth и запустил Firebase startup.
        // Если minimum window уже истёк — hide происходит сразу; если нет —
        // таймер выше снимет cover позже. Сам auth этим не задерживается.
        if (!inviteLaunch) {
            normalAppReady = true;
            maybeHideNormalStartupCover();
        }
