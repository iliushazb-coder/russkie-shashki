// Сообщаем Telegram, что наше приложение готово к работе
if (window.Telegram && window.Telegram.WebApp) {
    Telegram.WebApp.ready();
    Telegram.WebApp.expand();
}

// ===== ЗВУКИ (генерируются прямо в браузере, без внешних файлов) =====
...
// ===== СЧЁТЧИКИ СЪЕДЕННЫХ ШАШЕК =====

let capturedDarkCount = 0; // сколько чёрных шашек съедено (твоя добыча)
let capturedLightCount = 0; // сколько твоих белых шашек съел соперник
...
squares.forEach(square => {
    square.addEventListener("click", () => {
        ...
        if (opponentsOnPath === 0) {
            ...
            // Обновляем счётчик в зависимости от того, чью шашку съели
